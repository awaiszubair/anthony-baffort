...
serve(async (req) => {
  ...
  try {
    ...
    // Use Gemini image model to outpaint/expand the image
    ...
    const expandedImageUrl = data.choices?.[0]?.message?.images?.[0]?.image_url?.url;
    ...
    return new Response(JSON.stringify({ imageUrl: expandedImageUrl }), {
      status: 200,
      headers: { ...corsHeaders, "Content-Type": "application/json" },
    });
  } catch (e) {
    console.error("ai-outpaint error:", e);
    return new Response(
