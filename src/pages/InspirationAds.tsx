      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Bekijk advertenties van merken die je volgt voor creatieve inspiratie.
          </p>
        </div>
        ...
        {brands.length === 0 && !showAddForm && (
          <div className="mb-8 rounded-lg border border-border bg-card p-8 text-center">
            ...
            <p className="text-foreground font-medium mb-1">Nog geen merken toegevoegd</p>
            <p className="text-sm text-muted-foreground mb-4">
              Voeg merken toe om hun actieve advertenties te bekijken voor inspiratie.
            </p>
            <Button variant="outline" size="sm" onClick={() => setShowAddForm(true)} className="gap-1.5">
              <Plus className="h-3.5 w-3.5" />
              Eerste merk toevoegen
            </Button>
          </div>
        )}
        ...
        {brands.length > 0 && !selectedBrandId && !loading && ads.length === 0 && (
          <div className="text-center py-12">
            <p className="text-muted-foreground">Klik op een merk hierboven om advertenties te laden.</p>
          </div>
        )}
                ...
                <div className="aspect-square overflow-hidden bg-muted relative">
                  ...
                  {ad.snapshot_url && (
                    <a
                      ...
                      className="absolute top-2 right-2 p-1.5 rounded-md bg-card/80 hover:bg-card text-foreground transition-colors opacity-0 group-hover:opacity-100"
                    >
        ...
        {!loading && ads.length === 0 && !error && selectedBrandId && (
          <div className="text-center py-12">
            <p className="text-muted-foreground">Geen advertenties gevonden voor dit merk.</p>
          </div>
        )}
      </div>
    </div>
